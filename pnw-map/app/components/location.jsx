...
function getLocation(callback) {
  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      console.log(
        `High accuracy location - Lat: ${latitude}, Lng: ${longitude}, Accuracy: ${accuracy}m`
      );

      // Increase accuracy threshold for indoor positioning
      if (accuracy <= 150) {
        // Increased from 100m to 150m
        ...
        navigator.geolocation.getCurrentPosition(
          (lowAccPos) => {
            ...
            console.log(
              `Low accuracy location - Lat: ${latitude}, Lng: ${longitude}, Accuracy: ${accuracy}m`
            );
            const buildingName = determineBuilding(latitude, longitude);
            if (callback) callback(buildingName);
          },
...
function determineBuilding(latitude, longitude) {
  ...
  if (
    latitude >= SULBP3.lat &&
    latitude <= SULBP1.lat &&
    longitude >= SULBP1.lng &&
    longitude <= SULBP2.lng
  ) {
    return "SULB";
  } else if (
    latitude >= GYTEP3.lat &&
    latitude <= GYTEP1.lat &&
    longitude >= GYTEP1.lng &&
    longitude <= GYTEP2.lng
  ) {
    return "Gyte";
  } else if (
    latitude >= POTTERP3.lat &&
    latitude <= POTTERP1.lat &&
    longitude >= POTTERP1.lng &&
    longitude <= POTTERP2.lng
  ) {
    return "Potter";
  } else if (
    latitude >= POWERSP3.lat &&
    latitude <= POWERSP1.lat &&
    longitude >= POWERSP1.lng &&
    longitude <= POWERSP2.lng
  ) {
    return "Powers";
  } else if (
    latitude >= CLOP3.lat &&
    ...
    latitude >= NILSP3.lat &&
    latitude <= NILSP1.lat &&
    longitude >= NILSP1.lng &&
    longitude <= NILSP2.lng
  ) {
    return "NILS";
  } else if (
    latitude >= PORTERP3.lat &&
    latitude <= PORTERP1.lat &&
    longitude >= PORTERP1.lng &&
    longitude <= PORTERP2.lng
  ) {
    return "Porter";
  } else if (
    latitude >= OFFICEP3.lat &&
    latitude <= OFFICEP1.lat &&
    longitude >= OFFICEP1.lng &&
    longitude <= OFFICEP2.lng
  ) {
    return "Office of Admissions";
  } else if (
    latitude >= FITNESSP3.lat &&
    latitude <= FITNESSP1.lat &&
    longitude >= FITNESSP1.lng &&
    longitude <= FITNESSP2.lng
  ) {
    return "Fitness Center";
  } else if (
    latitude >= COUNSELINGP3.lat &&
    latitude <= COUNSELINGP1.lat &&
    longitude >= COUNSELINGP1.lng &&
    longitude <= COUNSELINGP2.lng
  ) {
    return "Counseling Center";
  } else if (
    latitude >= ANDERSONP3.lat &&
    latitude <= ANDERSONP1.lat &&
    longitude >= ANDERSONP1.lng &&
    longitude <= ANDERSONP2.lng
  ) {
    return "Anderson";
  }
...
export const COUNSELINGP1 = {
  lat: 41.579594338742616,
  lng: -87.47526740499988,
}; // Top Left
export const COUNSELINGP2 = { lat: 41.57959132916529, lng: -87.47498174976117 }; // Top Right
export const COUNSELINGP3 = {
  lat: 41.579230178868485,
  lng: -87.47500454853608,
}; // Bot Right
